...
class Plots:
    ...
    p_type = {'normal' : {'grid_layout' : (3,1), 'bar_width' : 0.8, 'outer_grid_heights' : [8,4,8], 'figsize' : (11,11)},
          'inhibition' : {'grid_layout' : (2,1), 'bar_width' : 0.4,  'outer_grid_heights' : [6,4], 'figsize' :(10,11)},
    ...
    def plot_metrics(self, correlation):
        ...
        if self.project_type == 'inhibition':
            ...
            ticks1, ticks2 = self.states_mol, states 
            ticklabels1, ticklabels2 = self.state_labels_mol, state_labels 
        ...
        print(ticks1, ticks2)
        print(ticklabels1, ticklabels2)

        vmin, vmax = 0, 1
        print(self.mol2[self.mol], self.mol, np.shape(correlation))    
        corr_img = ax.imshow(correlation / _vmax, cmap=cmap, vmin=vmin, vmax=vmax, aspect='auto')
    ...
    def plotBindingProfileAndCombinatorial(self):
        ...
        for project_type, projects in self.supra_project.items():
            ...
            for idx_mol, (mol, project) in enumerate(projects.items()):
                ...
                #TODOTODOTODOTODOTODOTODOTODOT for inhib and water. make function maybe
                combinatorial_df = combinatorial[project_type][mol]
                _states = np.unique(combinatorial_df.values.flat)
                self.states = {idx : input_states[project_type][k] for idx, k in enumerate(_states)}
                print(self.states)
                
                combinatorial_df.replace(to_replace=_states, value=self.states.keys())
                ...
                if project_type == 'inhibition':
                    ...
                    #TODO: fix dims
                    _states_mol = np.unique(df_comb_inhib.values.flat)
                    self.state_labels_mol = [v for k, v in self.states.items() if k in self.states_mol]
                    ...
                    #plot_combinatorial
                    print(inib_mol, self.states_mol, state_sampling_fraction)
                    self.ax_comb2.bar(self.states_mol, state_sampling_fraction, color=color2)
            ...
            if project_type == 'water':
                ...
                for idx_mol_w, (mol_w, iterables_mol_w2) in enumerate(self.mol_water.items()):
                    ...
                    _base_states_mol = np.unique(combinatorial_df.values.flat) #input_states['normal']
                    base_states_mol = {idx : input_states[project_type][k] for idx, k in enumerate(_base_states_mol)}
                    comb_df = combinatorial[project_type][mol]
                    comb_df = comb_df.iloc[:, comb_df.columns.get_level_values('l2') == mol_w]
                    
                    _states = np.unique(comb_df.values.flat)
                    
                    self.states = {idx : input_states[project_type][k] for idx, k in enumerate(_states)}
                    print(self.states)
                    comb_df.replace(to_replace=_states, value=self.states.keys())       
