        ...
        for project_type, projects in self.supra_project.items():
            ...
            if project_type == 'inhibition':
                ...
                if water_mode == 'combinatorial' :
                    ...
                    self.regions = regions[f'{project_type}_double']
                    ...
                    for _mol in mols:       
                        ...
                        for idx, it in enumerate(its):
                            lag = lags[project_type][_mol][idx]
                            disc_traj = data.iloc[:, data.columns.get_level_values('l3') == it].values.T.astype(int).tolist() #data[it]
    ...
    def plot_kinetics(self, input_data_mols, paths_to_plot=-1):
        ...
        for idx_mol, (mol, input_data) in enumerate(input_data_mols.items()):
            ...
            if self.water_mode != 'combinatorial':
                its = self.supra_project['normal'][mol].parameter
            else:
                if mol == 'H2O':
            ...
            for idx, ((name, data), model, mfpt) in enumerate(zip(*data_elements)):
                ...
                if self.project_type != 'inhibition':
                    ...
                    if mol == ('BeOH' or 'BuOH') and idx == 0:
                        break
                    elif mol == ('BeOH' or 'BuOH') and idx != 0:                        
                        color =  'red' #'white'
                ...
                self.plot_mfpt(mfpt, mol, idx, color, _state_labels, axes_mfpt)
            ...
            #print(np.asarray(fluxes)[:,0].dtype(int))
            
            #its = [self.supra_project['normal'][mol].parameter[idx] for idx in np.asarray(fluxes)[:,0]]
            #its = [self.supra_project['normal'][mol].parameter[it] for it in self.supra_project['normal'][mol].parameter if it in ]
...
# =============================================================================
            ...
            locmax = LogLocator(base=10.0, numticks=12) #subs=(0.2, 0.3, 0.5, 0.7, 1)
    ...
    def plot_mfpt(self, mfpt, mol, idx, color, _state_labels, axes_mfpt):
        ...
        #TODOTODOTODOTODO: check labels!!!!!!! water they are the same???

        label_max = ' -> '.join([_state_labels[i] for i in ind_max])
        label_min = ' -> '.join([_state_labels[i] for i in ind_min])
        if self.project_type == 'water':
            print(ind_min, ind_max, label_min, label_max)
            print(mfpt)
